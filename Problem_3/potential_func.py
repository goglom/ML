...
class PotentialFunctionClassifier:
    ...
    def _remeber(self, train_x: np.ndarray, train_y: np.ndarray) -> None:
    ...
    def fit(self, train_x: np.ndarray, train_y: np.ndarray, epochs: int=1) -> None:
    ...
    def predict(self, x: np.ndarray):
        ...
        diff = test_x[:, np.newaxis, :] - self.train_x[np.newaxis, non_zero_mask, :]
        distances = np.sqrt(np.sum((diff**2), -1))
        distances = self.charges[non_zero_mask] * self.kernel(distances / self.widnow_size)
        table = np.zeros((test_x.shape[0], len(self.classes)))

        for c in self.classes:
            table[:, c] = np.sum(distances[:, self.train_y[non_zero_mask, ...] == c], axis=1)
